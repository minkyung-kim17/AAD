...
class ODDSDataset(Dataset):
    ...
    def __init__(self, root: str, dataset_name: str, ratio_pollution=None, 
        ...
        scaler = StandardScaler().fit(X_train.astype(np.float))
        X_train_stand = scaler.transform(X_train.astype(np.float))

        minmax_scaler = MinMaxScaler().fit(X_train_stand.astype(np.float))
        X_train_stand = minmax_scaler.transform(X_train_stand.astype(np.float))

        if ratio_pollution == 0:  # ratio_pollution
            X_test_stand = scaler.transform(X_test.astype(np.float))

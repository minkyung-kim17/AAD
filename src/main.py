...
@click.argument('ratio_pollution', type=float)  # Contamination ratio of unlabeled training dataset.
...
def main(dataset_name, ratio_pollution, al_loss, qs_type, ps_abnormal, seed, n_repeat,
    ...
    for n_repeat_idx in range(n_repeat):  # for random seeds
        ...
        for al_iter_idx in range(n_al_iter+1):
            ...
            if pretrain:  ## Pretrain model on dataset (via autoencoder)
            ...
            if dataset_name in ['ionosphere', 'arrhythmia', 'glass']:  # for small dataset
                numQueries = 6
